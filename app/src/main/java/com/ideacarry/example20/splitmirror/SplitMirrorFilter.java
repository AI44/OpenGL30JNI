package com.ideacarry.example20.splitmirror;
...
public class SplitMirrorFilter extends BaseFilter {
    ...
    public void onCreate(Context context) {
        mProgram = new GLShaderProgram(new String(CommonUtils.readAssetFile(context, "example20/splitmirror/vs_split_mirror.glsl")),
                new String(CommonUtils.readAssetFile(context, "example20/splitmirror/fs_split_mirror.glsl")));
    ...
    public void onSizeChange(Context context, int viewWidth, int viewHeight, int cameraWidth, int cameraHeight, int degree, int flip) {
        if (mProgram != null) {
            if (viewWidth > 0 && viewHeight > 0) {
                mProgram.use();
                float[] rect = SplitFilter.getTextureOffset(viewWidth, viewHeight, COLUMN, ROW);
