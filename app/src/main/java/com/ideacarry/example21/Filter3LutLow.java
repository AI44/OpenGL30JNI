...
public class Filter3LutLow extends BaseFilter {
    public Filter3LutLow(Context context, Bitmap bmp, IResult<Bitmap> result) {
    ...
    public void doFilter(int w, int h, int textureId) {
        ...
        GLShaderProgram program = new GLShaderProgram(new String(CommonUtils.readAssetFile(mContext, "example21/filter3_lut_vertex.glsl")),
                new String(CommonUtils.readAssetFile(mContext, "example21/filter3_lut_low_fragment.glsl")));
        program.use();
