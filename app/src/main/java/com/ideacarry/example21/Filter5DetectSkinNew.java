...
public class Filter5DetectSkinNew extends BaseFilter {
    public Filter5DetectSkinNew(Context context, Bitmap bmp, IResult<Bitmap> result) {
    ...
    public void doFilter(int w, int h, int textureId) {
        ...
        GLShaderProgram program = new GLShaderProgram(new String(CommonUtils.readAssetFile(mContext, "example21/filter5_detect_skin_vertex.glsl")),
                new String(CommonUtils.readAssetFile(mContext, "example21/filter5_detect_skin_new_fragment.glsl")));
        program.use();
