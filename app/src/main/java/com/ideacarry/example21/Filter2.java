...
public class Filter2 extends BaseFilter {
    public Filter2(Context context, Bitmap bmp, IResult<Bitmap> result) {
    ...
    public void doFilter(int w, int h, int textureId) {
        ...
        GLShaderProgram program = new GLShaderProgram(new String(CommonUtils.readAssetFile(mContext, "example21/filter2_vertex.glsl")),
                new String(CommonUtils.readAssetFile(mContext, "example21/filter2_fragment.glsl")));
        program.use();
